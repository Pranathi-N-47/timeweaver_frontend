...
function renderConflicts() {
    ...
    mockConflicts.forEach(conflict => {
        ...
        // Animation class
        card.className = 'glass p-5 rounded-xl border-l-4 border-slate-700 hover:bg-slate-800/50 transition-all cursor-pointer animate-fade-in group relative';
        ...
        card.innerHTML = `
            <div class="flex items-start gap-4">
                    ...
                    <div class="flex items-center justify-between mb-1">
                        <h4 class="text-white font-semibold text-lg flex items-center gap-2">
                            ...
                            ${conflict.severity === 'critical' 
                                ? '<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-red-500 text-white tracking-wider">CRITICAL</span>' 
                                : '<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-500 text-slate-900 tracking-wider">WARNING</span>'}
                        </h4>
                        <span class="text-slate-500 text-xs font-mono">#${conflict.id}</span>
                    </div>
                    
                    <p class="text-slate-400 text-sm mb-3">${conflict.description}</p>
                    
                    <div class="flex flex-wrap items-center gap-4 text-xs text-slate-400">
                        <div class="flex items-center gap-1.5 bg-slate-800/50 px-2 py-1 rounded">
                            <i class="fa-regular fa-clock text-indigo-400"></i> ${conflict.time}
                        </div>
                        <div class="flex items-center gap-1.5 bg-slate-800/50 px-2 py-1 rounded">
                            <i class="fa-solid fa-location-dot text-indigo-400"></i> ${conflict.location}
                        </div>
                        <div class="flex items-center gap-1.5 bg-slate-800/50 px-2 py-1 rounded">
                            <i class="fa-solid fa-users text-indigo-400"></i> ${conflict.parties.join(', ')}
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2 opacity-0 group-hover:opacity-100 transition-opacity absolute right-4 top-1/2 -translate-y-1/2 md:static md:opacity-100 md:translate-y-0">
                    <button onclick="resolveConflict('${conflict.id}')" class="p-2 bg-green-500/10 hover:bg-green-500 text-green-500 hover:text-white rounded-lg transition-colors" title="Mark Resolved">
                        <i class="fa-solid fa-check"></i>
                    </button>
                    <button class="p-2 bg-indigo-500/10 hover:bg-indigo-500 text-indigo-400 hover:text-white rounded-lg transition-colors" title="View Details">
...
window.resolveConflict = function(id) {
    ...
    if (idx !== -1) {
        ...
        // Show toaster or notification (simple alert for now)
        // In a real app, use a nice toast
